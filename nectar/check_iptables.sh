...
# Version 0.5
# https://github.com/rhoml/check-iptables/blob/master/check_iptables_status.sh
#
# ---------------------------------------- License -----------------------------------------------------
#
...
CHKIPTBLS="`/sbin/iptables -L $table -n |wc -l`"
...
then
    echo "OK - Iptables are OK The Chain $table has $TOTRULES rules configured"
    # Nagios exit code 0 = status OK = green
    exit 0
else
    echo " CRITICAL - Iptables are CRITICAL The Chain $table has $TOTRULES rules configured"
